...
const Footer = ({ lang }: { lang: TSupportedLanguages }) => {
  const content = {
    tagline: {
    ...
    <footer className="max-w-[100vw] px-8 md:px-12 lg:px-24 pt-4 md:pt-12 pb-8 flex flex-col items-center footer">
      <div className="flex flex-col gap-16 md:gap-4 md:flex-row justify-between w-full">
        <div className="w-full grid place-items-center md:place-items-start md:flex-1">
          ...
          <p className="max-w-72 min-[400px]:max-w-80 md:max-w-64 text-sm text-center sm:text-left text-white">
        ...
        <nav className="md:flex-1 grid place-items-center">
          <ul className="gap-8 flex flex-col items-center sm:flex-row px-2">
            {NAV_LINKS.map((link) => (
              <li key={link.target}>
                <MenuItem label={link.label[lang]} target={link.target} textClassName="text-white" />
              </li>
            ))}
          </ul>
        </nav>
          ...
          <div>
            ...
            <div className="flex gap-2">
              <a href="https://www.linkedin.com/in/remyzins/" target="blank">
                <Image src="/LinkedIn-bright.svg" alt="LinkedIn" width={30} height={30} />
              </a>
              <a href="https://github.com/LaniakeanDev" target="blank">
                <Image src="/GitHub-bright.svg" alt="GitHub" width={30} height={30} />
              </a>
            </div>
          </div>
        </div>
